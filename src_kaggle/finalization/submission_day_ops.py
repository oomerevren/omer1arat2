"""Operational markdown outputs for race-day decisions."""
from __future__ import annotations
from pathlib import Path


def write_operational_decision_table(path: str | Path = "reports/final/final_operational_decision_table.md") -> None:
    ...
    lines = [
        ...
        "", "## Hard stop risk flags", "", "- PUBLIC_UP_OOF_DOWN", "- PUBLIC_UP_CLASS0_DOWN", "- PRIVATE_UNSAFE_CANDIDATE", "- missing submission validator report", "- config final_mode is not true", "- manifest/config checksum mismatch", "", "## Default final answer", "", "If the team cannot reach consensus, keep **Family A — Balanced / Safest**.",
    ]
    p.write_text("\n".join(lines) + "\n", encoding="utf-8")
